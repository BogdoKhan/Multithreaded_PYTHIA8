...
void protoxy_Histos(){
	...
	/*TCanvas* c4 = new TCanvas("MB_delt", name, 0., 0., 1000., 800.);
	c4->cd(0);
	gPad->SetLogy();
	gStyle->SetOptStat(0);
	gStyle->SetOptFit(0);
	nhpt_5502->SetDirectory(0);
	nhpt_5502->GetXaxis()->SetRangeUser(20., 50.);
	nhpt_5502->Draw();*/


/*	dnhpt_5->Fit("lin_form_11", "R", "" , 20, 50);
	TF1* fitres_ratio1 = (TF1*)dnhpt_5->GetListOfFunctions()->FindObject("lin_form_11");
	...
	nhpt_14502->Fit("expoconst","R0","",20.,50.);

	TF1 *fit_pl1 = nhpt_14502->GetFunction("expoconst");
	Double_t chi2_fit_pl1 = fit_pl1->GetChisquare();
	Double_t const_fit1 = fit_pl1->GetParameter(0);
	Double_t slope1 = fit_pl1->GetParameter(1);
	cout << "Chi2: " << chi2_fit_pl1 << endl;

	const_form_exp1->SetParName(0, "Shift");
	const_form_exp1->SetParName(1, "Slope");
	const_form_exp1->SetParameter(1, slope);
	dnhpt_14->Fit("lin_form_11", "R", "" , 20, 50);
	TF1* fitres_ratio11 = (TF1*)dnhpt_14->GetListOfFunctions()->FindObject("lin_form_11");
	Double_t fit_ratio11 = fitres_ratio11->GetParameter(0);
	Double_t fiterr11 = fitres_ratio11->GetParError(0);
	//cout << endl << "Calculated ratio: " << calc_rat << endl;
	cout << endl << "Calculated ratio: " << fit_ratio11 << " " << fiterr11 << endl;
	...
	nhpt_21502->Fit("expoconst","R0","",20.,50.);

	TF1 *fit_pl2 = nhpt_21502->GetFunction("expoconst");
	Double_t chi2_fit_pl2 = fit_pl2->GetChisquare();
	Double_t const_fit2 = fit_pl2->GetParameter(0);
	Double_t slope2 = fit_pl2->GetParameter(1);
	cout << "Chi2: " << chi2_fit_pl2 << endl;

	const_form_exp1->SetParName(0, "Shift");
	const_form_exp1->SetParName(1, "Slope");
	const_form_exp1->SetParameter(1, slope);
	dnhpt_21->Fit("lin_form_11", "R", "" , 20, 50);
	TF1* fitres_ratio12 = (TF1*)dnhpt_21->GetListOfFunctions()->FindObject("lin_form_11");
	Double_t fit_ratio12 = fitres_ratio12->GetParameter(0);
	Double_t fiterr12 = fitres_ratio12->GetParError(0);
	//cout << endl << "Calculated ratio: " << calc_rat << endl;
	cout << endl << "Calculated ratio: " << fit_ratio12 << " " << fiterr1 << endl;*/
